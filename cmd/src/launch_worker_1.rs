...
async fn main() -> TribResult<()> {
    ...
    eprintln!("Validation error Net 3 Worker 1: {}", net3.cost(&validation_data));
    eprintln!("Validation error Net 4 Worker 1: {}", net4.cost(&validation_data));
    for (x, y) in &training_data[point..batch_size] {
        ...
        println!("Net 3 Worker 1: {}\t{}\t{}", x, original_fn(*x), net3.eval(*x));
        println!("Net 4 Worker 1: {}\t{}\t{}", x, original_fn(*x), net4.eval(*x));
    }
    Ok(())
}